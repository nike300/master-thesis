...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "08.04.26_EckenPruefung.csv")
...
def get_true_window_normal_and_corners(obj, tower_center_2d):
    """
    Findet die physikalische Normale UND die Eckpunkte der größten Fläche (Scheibe).
    Gibt ein Tuple zurück: (Normalen-Vektor, Liste der Eckpunkt-Vektoren in Weltkoordinaten)
    """
    mesh = obj.data
    ...
    # --- NEU: ECKPUNKTE AUSLESEN ---
    # Wir holen die 3D-Weltkoordinaten aller Eckpunkte dieser spezifischen Glas-Fläche
    corners_world = []
    for loop_idx in largest_face.loop_indices:
        vertex_idx = mesh.loops[loop_idx].vertex_index
        local_coord = mesh.vertices[vertex_idx].co
        world_coord = matrix @ local_coord
        corners_world.append(world_coord)
...
def run_final_simulation():
    ...
    print("=" * 60)
    print("STARTE MATHE-SIMULATION MIT ECKEN-PRÜFUNG")
    print("=" * 60)
    ...
    # Wir speichern jetzt Normale UND Ecken für jedes Fenster
    sensor_geometry = []
    for s in sensors:
    ...
    for day, month in SIMULATION_DATES:
        ...
        for total_minutes in range(START_HOUR * 60, (END_HOUR * 60) + 1, MINUTES_STEP):
            ...
            for i, sensor in enumerate(sensors):
                ...
                # Daten aus unserer vorberechneten Liste holen
                normal = sensor_geometry[i]["normal"]
                corners = sensor_geometry[i]["corners"]
                ...
                # --- NEU: MULTI-RAYCAST FÜR ECKEN ---
                is_completely_shaded = True # Wir gehen erstmal von Schatten aus
                
                for corner_loc in corners:
                    ray_start = corner_loc + (direction * 0.3) 

                    hit, _, _, _, hit_obj, _ = bpy.context.scene.ray_cast(
                        depsgraph, ray_start, direction, distance=1000.0 
                    )
                    
                    # Wenn der Strahl KEIN Mesh trifft, scheint die Sonne auf DIESE Ecke!
                    if not (hit and hit_obj.type == 'MESH'):
                        is_completely_shaded = False
                        break # WICHTIG: Schleife abbrechen! Eine Ecke reicht für "Sonne".
                
                # Auswertung für das Fenster
