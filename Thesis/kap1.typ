...
- Medienbrüche und manuelle Datenaufbereitung: Kommerzielle Dienstleister fordern häufig proprietäre CAD-Formate (wie Revit oder DWG) mit strikten Dateigrößenlimits von beispielsweise 250 Megabyte. Zudem muss die Fachplanung das Modell oftmals manuell bereinigen, etwa durch die Entfernung von Innenwänden oder Inventar, bevor es übermittelt werden kann. Ein durchgängiger, IFC-basierter Workflow zielt hingegen darauf ab, relevante Entitäten wie `IfcWindow` oder `IfcCurtainWall` automatisiert aus dem Open@bim#[]-Modell auszulesen, ohne durch künstliche Dateigrößen limitiert zu sein.
